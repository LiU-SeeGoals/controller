...
type GameAnalysis struct {
	...
	predictions [][]Zone // 2D array of predictions
	fieldLength float32
	fieldWidth  float32
	zoneSize    float32
}
...
func (an *GameAnalysis) calculateTime(robots [gamestate.TEAM_SIZE]*gamestate.Robot, i, j int) float64 {
	time := math.Inf(1)
	...
	for _, robot := range robots {
		...
		time = distance / robot.GetSpeed()
	}
	return time
}

func (an *GameAnalysis) calculateTimeAdvantage(gamestateObj *gamestate.GameState, i, j int) float64 {
	timeYellow := an.calculateTime(gamestateObj.GetYellowRobots(), i, j)
	timeBlue := an.calculateTime(gamestateObj.GetBlueRobots(), i, j)
...
func (an *GameAnalysis) updateZones(gamestateObj *gamestate.GameState) {
	...
	for i := 0; i < len(an.zones); i++ {
		for j := 0; j < len(an.zones[i]); j++ {
			...
			an.zones[i][j].timeAdvantage = an.calculateTimeAdvantage(gamestateObj, i, j)
		}
	}
}
